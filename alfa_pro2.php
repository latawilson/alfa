  ...
  if($_GET['action']=='delete')
  {
    foreach ($_SESSION['add_carro'] AS $key => $value)
    {
      if($value['item_id'] == $_GET['id'])
      {
        ...
        echo '<script>window.location="alfareria.php"</script>';  
...
<head>
  ...
  <script type="text/javascript">
    ...
    function openOfertas(){
      $(".ofertasventana").slideDown("slow"); 
    }
    function closeOfertas(){
     $(".ofertasventana").slideUp("fast"); 
   }
      ...
      <div class="site-section">
       <div class="informacion-pro">
        <div class="container">
          <div class="row">
            <div class="col-md-6">
              <form method="post" action="alfa_pro2.php?id_al=<?php echo($row['id_al']);?>;alfa_pro2.php?action=add&id=2" >
                <img id="imagen_pro" alt="Image" class="img-fluid" width="300"  height="300" style="border: solid 1px red;">
                <div class="fb-like" data-href="http://localhost/alfafinal/alfa_pro2.php?id_pro=<?php echo($row['id_pro']);?>" data-width="200" data-layout="button" data-action="like" data-size="large" data-show-faces="true" ></div><hr style="width: 250px;margin-left: 20px; ">
              </div>
              <div class="col-md-6">  
                <h2 class="text-black" id="nombre_pro"  > </h2>
                <hr>
              ...
              <div class="facebook">
                ...
                <div class="facebook" style="padding: 30px;">
                  ...
                  <br>
                  <div class="fb-comments" data-href="http://localhost/alfafinal/alfa_pro2.php?id_pro=<?php echo($row['id_pro']);?>" data-width="400" data-numposts="5"></div>
                </center>
              </div>
    ...
    if(!empty($_SESSION["add_carro"]))
    {
      ...
      foreach($_SESSION["add_carro"] as $keys => $values)
      {
        ...
        <tr>
          ...
          <!-- alfa_pro2.php?action=delete&id=<?php echo $values["item_id"]; ?> -->
          <td><a href="alfa_pro2.php?action=delete&id=<?php echo $values["item_id"]; ?>"><span class="text-danger">X</span></a></td>
        </tr>
        <?php
...
<div  class="site-blocks-cover2 "style="background-image: url(images/fondo_productos.png);   " data-aos="fade">
  <div class="container">
   <FONT FACE="arial black"  SIZE=2 >
    <div class="hero-text">
     <h1 class="mb-2" style="margin-left: 100px; color:#FF3300" ><?php echo($row['nombre_al']); ?></h1>
...
<div class="site-section " data-aos="fade">
 <div class=" site-section-heading text-center pt-4">
  ...
  <hr>
  <a href="javascript:openOfertas();" > <h3 > <u style="color: #F2C35F;"><i style="color: #F2C35F;">Ver Ofertas de la Alfarer&iacutea</i></u></a></h3>
</div>


<div class="ofertasventana">
  <div class="ofertasproductos">
    <strong><a class="cerrar-vent" href="javascript:closeOfertas();">Cerrar</a></strong> 
    <br>

    <?php
    require_once ("clases/cls_producto.php");
    $obj_producto = new producto ();
    $resultalfa= $obj_producto->consultpromocion($_GET['id_al']);
    ?>
    <div class="container" >
      <div class="row" >
        <div size="<?php echo($row['idate(format)_al']); ?>" >
          ...
          while($row= mysql_fetch_array($resultalfa))
          {
            ...
            <div class="contenedor-peq "  style=" width: 250px;  border: solid 1px #FF3300; ">
             <div class="info1">
               <center>
                <p class="titulo" style="margin-left: -100PX; "><P class="titulo"><?php echo($row['nombre_pro']); ?> </P>
                  <img src="images/of1.png" width="55" height="55"  style=" margin-top: -32px; margin-left: -185px;">
                </p> </center> 

              </div>
              <form method="post" action="alfa_pro2.php?id_al=<?php echo($row['alfareria_id_al']);?>;alfa_pro2php?action=add&id=<?php echo $row['id_pro'] ; ?> javascript:openCarrito();  " >
                <?php
                ...
                <div class="info">
                  ...
                  <input style="display: none" type="hidden" name="hidden_precio" value="

                  $<?php echo( -($row['precio_pro'] *$row['descuento_prom']/100))+$row['precio_pro']; ?>
                  " />
                  <input style="display: none" type="hidden" name="hidden_id_pro" value="<?php echo $row['id_pro'];?>" />
                  <input  type="hidden" name="subtotal" value="<?php echo $row['cantidad_pro'];?> - cantidad" />
  ...
  <div class="row" >
    <div size="<?php echo($row['id_al']); ?>" >
      ...
      while($row= mysql_fetch_array($resultalfa))
      {
        ...
        <div class="contenedor-peq "  style=" width: 240px;  border: solid 1px #FF3300; ">
         <div class="info1">
           <center>
            <p class="titulo"><?php echo($row['nombre_pro']); ?></p> </center> 
            
          </div>
          <form method="post" action="alfa_pro2.php?id_al=<?php echo($row['alfareria_id_al']);?>;alfa_pro2.php?action=add&id=<?php echo $row['id_pro'] ; ?> javascript:openCarrito();  " >
            <?php
            ...
            <div class="info">
              ...
              <input style="display: none" type="hidden" name="hidden_id_pro" value="<?php echo $row['id_pro'];?>" />
              <input  type="hidden" name="subtotal" value="<?php echo $row['cantidad_pro'];?>" />
  ...
  function cargarDetalle(img){
       ...
       while($row= mysql_fetch_array($result))
       {
        ...
        if(img.id==<?php echo ($row['id_pro']);?>){
         ...
         $("#descripcion_pro").html("Descripcion: <?php echo ($row['descripcion_pro']);?>");
         $("#precio_pro").html("  <?php echo ($row['precio_pro']);?>  $"); 
          //$("#i_pro").html("  <?php echo ($row['precio_pro']);?>  $");          
        }
        <?php 
      }
      ?>
